...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/04 15:31:22 by udraugr-          #+#    #+#             */
/*   Updated: 2021/01/04 15:31:33 by udraugr-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			init_hash_sha512(uint8_t endian, t_hash *hash)
{
	hash->hash32 = NULL;
	if (!(hash->hash64 = ft_memalloc(sizeof(uint64_t) * 8)))
		ft_exit_malloc_crash();
	hash->hash64[0] = 0x6a09e667f3bcc908;
	hash->hash64[1] = 0xbb67ae8584caa73b;
	hash->hash64[2] = 0x3c6ef372fe94f82b;
	hash->hash64[3] = 0xa54ff53a5f1d36f1;
	hash->hash64[4] = 0x510e527fade682d1;
	hash->hash64[5] = 0x9b05688c2b3e6c1f;
	hash->hash64[6] = 0x1f83d9abfb41bd6b;
	hash->hash64[7] = 0x5be0cd19137e2179;
	if (endian == BIG)
		ft_hash_swap_byte(hash);
	hash->size = 8;
}

static void			init_word_sha512(char *input_str, uint8_t endian,
...
char				*ft_sha512(t_input *input)
{
	...
	init_hash_sha512(input->endian, &hash);
	init_word_sha512(input_str, input->endian, &word, input);
	ft_strdel(&input_str);
	size = 0;
