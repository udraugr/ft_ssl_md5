...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/04 15:30:01 by udraugr-          #+#    #+#             */
/*   Updated: 2021/01/04 15:30:21 by udraugr-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			init_hash_sha384(uint8_t endian, t_hash *hash)
{
	hash->hash32 = NULL;
	if (!(hash->hash64 = ft_memalloc(sizeof(uint64_t) * 8)))
		ft_exit_malloc_crash();
	hash->hash64[0] = 0xCBBB9D5DC1059ED8;
	hash->hash64[1] = 0x629A292A367CD507;
	hash->hash64[2] = 0x9159015A3070DD17;
	hash->hash64[3] = 0x152FECD8F70E5939;
	hash->hash64[4] = 0x67332667FFC00B31;
	hash->hash64[5] = 0x8EB44A8768581511;
	hash->hash64[6] = 0xDB0C2E0D64F98FA7;
	hash->hash64[7] = 0x47B5481DBEFA4FA4;
	if (endian == BIG)
		ft_hash_swap_byte(hash);
	hash->size = 8;
}

static void			init_word_sha384(char *input_str, uint8_t endian,
...
char				*ft_sha384(t_input *input)
{
	...
	init_hash_sha384(input->endian, &hash);
	init_word_sha384(input_str, input->endian, &word, input);
	ft_strdel(&input_str);
	size = 0;
