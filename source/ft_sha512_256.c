...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/04 15:32:01 by udraugr-          #+#    #+#             */
/*   Updated: 2021/01/04 15:32:02 by udraugr-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			init_hash_sha512_256(uint8_t endian, t_hash *hash)
{
	hash->hash32 = NULL;
	if (!(hash->hash64 = ft_memalloc(sizeof(uint64_t) * 8)))
		ft_exit_malloc_crash();
	hash->hash64[0] = 0x22312194FC2BF72C;
	hash->hash64[1] = 0x9F555FA3C84C64C2;
	hash->hash64[2] = 0x2393B86B6F53B151;
	hash->hash64[3] = 0x963877195940EABD;
	hash->hash64[4] = 0x96283EE2A88EFFE3;
	hash->hash64[5] = 0xBE5E1E2553863992;
	hash->hash64[6] = 0x2B0199FC2C85B8AA;
	hash->hash64[7] = 0x0EB72DDC81C52CA2;
	if (endian == BIG)
		ft_hash_swap_byte(hash);
	hash->size = 8;
}

static void			init_word_sha512_256(char *input_str, uint8_t endian,
									t_word32 *word, t_input *input)
{
	...
	l = input->length;
	mod64 = (l + 1) % 128;
	nl = (mod64 > 120) ? l + 1 + 128 - (mod64 - 120) : l + 1 + 120 - mod64;
	if (!(word->buf = (unsigned char *)ft_strnew(nl + 16)))
		ft_exit_malloc_crash();
...
static void			compute_chunk(uint64_t *buf, t_hash *hash)
{
	uint64_t		th[8];
	uint64_t		buf64[80];
	uint64_t		i;
	...
	i = 0;
	while (++i <= 8)
		th[i - 1] = hash->hash64[i - 1];
	i = 0;
	while (i < 80)
	{
		if (i < 16)
			buf64[i] = swap_uint64(buf[i]);
		else
			buf64[i] = buf64[i - 16] + S0_64(buf64[i - 15])
						+ buf64[i - 7] + S1_64(buf64[i - 2]);
		++i;
	}
	i = 0;
	while (i < 80)
...
char				*ft_sha512_256(t_input *input)
{
	...
	init_hash_sha512_256(input->endian, &hash);
	init_word_sha512_256(input_str, input->endian, &word, input);
	ft_strdel(&input_str);
	size = 0;
	while (size < word.size)
	{
		compute_chunk((uint64_t *)&word.buf[size], &hash);
		size += 128;
	}
	hash.size -= 4;
	hash_str = ft_hash_to_ascii(&hash);
	ft_memdel((void **)&hash.hash64);
